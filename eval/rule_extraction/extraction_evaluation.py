...
from eval.metrics.metrics import eval_retrieval_qa, eval_compilation_qa
...
def load_output_csv(model, question_type, overwrite_answers=False):
    ...
    if not os.path.exists(csv_name) or overwrite_answers:
        questions_pd = pd.read_csv(
            os.path.join(
                PROJECT_ROOT, "dataset/rule_extraction", f"rule_{question_type}_qa.csv"
            )
        )
...
def run_thread(model, question, context):
    if model == "llama-3-local":
        # Local Llama 3 model optimized for Apple Silicon
        device = (
            "mps"
    ...
    if model in ["gpt-4-1106-vision-preview", "gpt-4-1106-vision-preview+RAG"]:
...
def create_index():
    ...
    pdf_path = os.path.join(PROJECT_ROOT, "dataset/docs/FSAE_Rules_2024_V1.pdf")
...
def retrieve_context(index, question, top_k=10):
    if top_k == 0:
        ...
        txt_path = os.path.join(PROJECT_ROOT, "dataset/docs/rules_pdfplumber1.txt")
...
if __name__ == "__main__":
    ...
    if os.path.exists("index"):
        ...
        index = load_index_from_storage(
            storage_context, embed_model=OpenAIEmbedding(model="text-embedding-3-large")
        )
    else:
    ...
    for question_type in ["retrieval", "compilation"]:
        # models available: 'gpt-4-0125-preview+RAG', 'gpt-4-0125-preview', 'llama-2-70b-chat', 'llava-13b', 'gpt-4-1106-vision-preview+RAG', 'gpt-4-1106-vision-preview', 'llama-3-local'
        for model in ["llama-3-local"]:  # Changed to use local model
            ...
            for i, row in tqdm(
                ...
                desc=f"generating responses for " f"{question_type} with {model}",
            ):
                ...
                # Run through model
                if model in [
                    "llama-3-local",  # Added local model
                    "llama-2-70b-chat",
